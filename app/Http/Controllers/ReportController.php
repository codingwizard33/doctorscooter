...
//                $item['warehouse'] = $product['product_warehouse']->name ?? 'no warehouse';
...
//                    $item['quantity'] = $total_qty .' '.$product['unit']->ShortName;
    ...
    public function download_report_client_pdf(Request $request, $id)
    {
     ...
     public function eposSales()
     {
         $eposSales = EposSale::query()->get();
         $count = $eposSales->count();
...
//    public function eposSales(request $request)
//    {
//
//        $this->authorizeForUser($request->user('api'), 'stock_report', Product::class);
//
